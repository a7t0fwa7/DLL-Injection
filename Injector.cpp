...
int InjectDll(DWORD processId, const wstring& dllPath) {
    ...
    if (remoteDllPath == NULL) {
        CloseHandle(processHandle);
        return -3; // retorna caso a alocação falhar
    }
    ...
    if (!WriteProcessMemory(processHandle, remoteDllPath, dllPath.c_str(), pathLength, NULL)) {
        ...
        CloseHandle(processHandle);
        return -4; // retorna caso a gravação na memória do processo falhar
    }
    ...
    VirtualFreeEx(processHandle, remoteDllPath, 0, MEM_RELEASE); // libera a memória alocada
    CloseHandle(processHandle);
...
int main() {
    ...
    // parte para armazenar o diretório do executavel
